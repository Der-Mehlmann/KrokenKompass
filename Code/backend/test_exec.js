...
mockL.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png", {
    maxZoom: 22
}).addTo(map);

let START_RAUM = "7721_00_111"; // Fallback
const zielSuchwert = null;
const startSuchwert = null;
...
window.wechsleEtage = function (zielEtage) {
    ...
    if (routingPfad) {
        ...
        routingPfad.forEach((knotenId, index) => {
            ...
            if (raumEtage === zielEtage) {
                ...
                if (typ === "vertikal" && globaleCentroids[knotenId]) {
                    ...
                    let m = mockL.circleMarker([koord[1], koord[0]], {
                        radius: 7,
                        fillColor: "#ec4899",
                        color: "#fff",
                        weight: 2,
        ...
        if (koordinatenFuerDieseEtage.length > 0) {
            aktuelleRoutenLinie = mockL.polyline(koordinatenFuerDieseEtage, {
...
map.on('locationfound', function (e) {
    if (!userLocationMarker) {
        userLocationMarker = mockL.marker(e.latlng, {
            icon: mockL.divIcon({
